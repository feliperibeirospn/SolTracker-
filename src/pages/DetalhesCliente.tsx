...
import { MdEdit, MdArrowBack, MdAccountBalanceWallet, MdHistory, MdReceiptLong, MdPerson } from 'react-icons/md';
...
const DetalhesCliente: React.FC = () => {
  ...
  const loadData = async (clienteId: number) => {
    try {
      ...
      // For now, history is global, but we could filter it if we had client-specific logs
      // For this task, showing recent global history or keeping it ready for specific filter
      const h = await HistoricoService.getAll();
      setHistoricos(h.slice(0, 10)); // Top 10 recent
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <section style={{ backgroundColor: 'var(--surface-color)', padding: '1.5rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            ...
            <p><strong>Data Início:</strong> {format(cliente.dataInicio, 'dd/MM/yyyy')}</p>
          </div>
        </section>
